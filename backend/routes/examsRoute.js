...
router.post("/add-question-to-exam", authMiddleware, async (req, res) => {
    ...
    //   if (!req.body.exam) {
    //     return res.status(400).send({ message: "Exam ID is required", success: false });
    //   }
    ...
    //   console.log("Saved Question:", question);
      ...
      // Find the exam and add the question ID to the exam's questions array
      const exam = await Exam.findById(req.body.exam);
    //   if (!exam) {
    //     console.error("Exam not found with ID:", req.body.exam);
